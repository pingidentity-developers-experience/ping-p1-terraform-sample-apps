                            ...
                            <TabPane tabId='2'>
                                ...
                                <Row className='p-3'>
                                    ...
                                    <Col md={8}>
                                        ...
                                        {/* PingOne Fraud Use Case - Coupon Abuse
                                            {this.state.selectedItem.model === 'C515C-V' && <Row>
                                            <Col md={6} className='text-right text-danger'>{this.state.invalidCouponMessage}</Col>
                                            <Col md={3} className='text-right'>
                                            <p>
                                                <Input
                                                    onChange={(e) => {
                                                        console.warn('Typing coupon code...');
                                                        this.setState({
                                                            couponCode: e.target.value,
                                                            invalidCouponMessage: '',
                                                            selectedItem: { ...this.state.selectedItem, finalTotal: data.productsClickable[0].finalTotal},
                                                            validCoupon: false
                                                        });
                                                    }}
                                                    type='text'
                                                ...
                                                <p>
                                                    <Button
                                                        onClick={async () => {
                                                            ...
                                                            const selectedItem = { ...this.state.selectedItem, finalTotal: data.productsClickable[0].finalTotal};
                                                            let invalidCouponMessage = 'Invalid coupon.';

                                                            if (this.state.couponCode === 'PINGY100'){
                                                                // Check for coupon abuse
                                                                const isCouponAbuse = await this.isCouponAbuse();

                                                                if (isCouponAbuse){
                                                                    invalidCouponMessage = 'Coupon abuse detected.';
                                                                } else {
                                                                    console.warn('APPLY COUPON:', 'P1Fraud tracks coupon code use here.');
                                                                    // P1Fraud integration
                                                                    window._securedTouch.CHECKOUT.applyCoupon();
                                                                    selectedItem.finalTotal = data.productsClickable[0].discountTotal;
                                                                    validCoupon = true;
                                                                    invalidCouponMessage = '';
                                                                }
                                                            }
                                                            ...
                                                            this.session.setAuthenticatedUserItem("cart", JSON.stringify(selectedItem), "session");
