...
resource "docker_container" "local_bxr_app" {
  ...
  env = ["REACT_APP_HOST=${local.app_url}",
    ...
    "SSL_CERT_FILE=ping-p1-terraform-sample-apps/BXRetail-Terraform-pkg/bxretail-sample-app/certs/cert.pem",
    "SSL_KEY_FILE=ping-p1-terraform-sample-apps/BXRetail-Terraform-pkg/bxretail-sample-app/certs/key.pem",
    "PORT=${var.app_port}"
  ]
}